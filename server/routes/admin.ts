...
// ============ PULL FROM RAILWAY (one-shot data sync) ============
//
// Server-to-server pull: Hatch pod fetches Railway's /api/download-db using the
// shared SEED_SECRET, validates the SQLite header + integrity, backs up the
// existing /files/shared.db, and atomically swaps it in.
//
// This exists because the Hatch oauth2-proxy gateway clears auth cookies on
// POSTs from outside the cluster — so the usual /api/upload-db path is
// unreachable from curl. A GET endpoint dodges that, and an admin session
// cookie carries through unchanged.
//
// REMOVE THIS once Railway is fully decommissioned — it's a migration tool,
// not a permanent feature. (See [[dcc-railway-deprecation]].)
router.get('/pull-from-railway', async (req, res) => {
  try {
    // Same dual auth as /download-db: x-seed-secret OR admin cookie session
    const secret = req.headers['x-seed-secret']
    const hasSeedToken = SEED_SECRET && secret === SEED_SECRET
    const sid = getSessionIdFromRequest(req)
    const session = sid ? sessions.get(sid) : null
    ...
    if (!SEED_SECRET) {
      return res.status(500).json({ error: 'DCC_SEED_SECRET not configured on this pod' })
    }

    const railwayUrl = process.env.DCC_RAILWAY_URL || 'https://wandihub.up.railway.app'
    const resp = await fetch(`${railwayUrl}/api/download-db`, {
      headers: { 'X-Seed-Secret': SEED_SECRET },
    })
    if (!resp.ok) {
      return res.status(502).json({ error: `Railway returned HTTP ${resp.status}` })
    }
    const arrayBuf = await resp.arrayBuffer()
    const dbBytes = Buffer.from(arrayBuf)
    if (dbBytes.length < 100) {
      return res.status(502).json({ error: 'Railway response too small to be a database' })
    }
    const header = dbBytes.subarray(0, 16).toString('ascii')
    if (!header.startsWith('SQLite format 3')) {
      return res.status(502).json({ error: 'Railway response is not a valid SQLite file' })
    }

    const backupPath = DB_PATH + '.pre-pull-' + Date.now()
    fs.copyFileSync(DB_PATH, backupPath)
